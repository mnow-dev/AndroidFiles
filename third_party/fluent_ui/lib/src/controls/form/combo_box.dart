...
class _ComboBoxResizeClipper extends CustomClipper<RRect> {
  ...
  RRect getClip(Size size) {
    final selectedItemOffset = getSelectedItemOffset();
    // Patched (AndroidFiles): when the popup is shorter than one item —
    // empty list or mid-resize — the original bounds invert and
    // double.clamp throws "Invalid argument(s)". Guard both tweens.
    final maxTop = size.height - kComboBoxItemHeight;
    final top = Tween<double>(
      begin: maxTop <= 0.0 ? 0.0 : selectedItemOffset.clamp(0.0, maxTop),
      end: 0,
    );

    final bottom = Tween<double>(
      begin: size.height <= kComboBoxItemHeight
          ? size.height
          : (top.begin! + kComboBoxItemHeight).clamp(
              kComboBoxItemHeight,
              size.height,
            ),
      end: size.height,
